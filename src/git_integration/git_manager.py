...
class GitManager():
    ...
    def get_commits_since_last_release(self):
        commits_data = self.get_commits_log_since_last_release().split('\n')
        return self.get_organized_commits_dic(commits_data) # get from app config

    def __get_commits_log_since_last_release(self):
        return self.gitHandler.log("--date=short", f"--since={self.get_last_release_date()}", "--oneline", "--pretty=%h|%s|%cN|%ci")
